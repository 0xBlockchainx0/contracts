...
contract('GatewayContract',  function(accounts) {
    ...
    */
    ...
    it("Link The Gateway <-> Staking Contracts", async function() {
        var stakingContract = await StakingContract.deployed();
        var gatewayContract = await GatewayContract.deployed();
        // should be able to set the gateway contract the first time.
        console.log(await gatewayContract.setStakingContract(stakingContract.address));
        console.log(await stakingContract.setGatewayContract(gatewayContract.address));
        // Start the gateway Contract, default is paused
        console.log(await gatewayContract.startContract());
        //owner balance
        let balance = await web3.eth.getBalance(accounts[0])
        console.log("balance of owner before  " + web3.utils.fromWei(balance, 'ether'))
    });
  ...
  //********* * Step. 1 * *********
    it("Create Post", async function() {
       ...
       console.log(await gatewayContract.createPost(postId,0));
